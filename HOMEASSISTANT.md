...
### Alpine Linux
Both build and runtime stages use Alpine (musl libc) for consistency. Native modules like `sharp` are compiled with Alpine's `vips-dev` and run with `vips`.
...
### Node.js version
- `clawdbot-gateway/Dockerfile` (FROM node:XX-alpine)
- `Dockerfile.ha-addon` (FROM node:XX-alpine)

### Runtime dependencies
- Both Dockerfiles → `apk add` in runtime stage
